...
> **v1.1.12 status**: tagged in git but **not published to npm** — release.yml's required-checks gate failed before reaching `npm publish`. `npm install agentlint-ai@1.1.12` resolves to nothing; users skip from `1.1.11` directly to `1.1.13`. The git tag stays (tag-protection ruleset blocks deletion of immutable semver tags).
